...
interface SettingsActions {
  ...
  setSearchStrategy: (strategy: SearchStrategyType) => void;
}
...
const initialState: SettingsState = {
  ...
  theme: undefined,
  searchStrategy: SearchStrategyType.KEYWORD,
};
  ...
  persist(
    (set, get) => ({
      ...
      toggleMemoryVerseMode: () =>
        set({
          memoryVerseMode:
            get().memoryVerseMode === 'full_word'
              ? 'first_letter'
              : 'full_word',
        }),
      setMemoryVerseSortOption: (option) =>
        set({ memoryVerseSortOption: option }),
      setTheme: (theme) => {
